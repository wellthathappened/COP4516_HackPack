import java.lang.Math; import java.awt.geom.Line2D; import java.awt.geom.Path2D; import java.awt.geom.Point2D;
public class Geometry {
	public static void main(String[] args){
		double p1x = 0D, p1y = 0D, p1z = 0D, p2x = 0D, p2y = 0D, p2z = 0;
		double p3x = 0D, p3y = 0D, p4x = 0D, p4y = 0D;
		double circX = 0D, circY = 0D, circR = 0D;
		...
		Vect vect1 = new Vect(p1x, p1y, p1z), vect2 = new Vect(p2x, p2y, p2z);
		Vect plane1 = new Vect(p1x, p1y, p1z), plane2 = new Vect(p1x, p1y, p1z), plane3 = new Vect(p1x, p1y, p1z);
	...
	public static boolean intersectLineSegCircle(Line2D line, Point2D circle, double r){
		return !(line.getP1().distance(circle) > r) || !( line.getP2().distance(circle) > r); }
	...
	public static boolean intersectLinePlane3(Vect v1, Vect v2, Vect p1, Vect p2, Vect p3) {
		...
		if(Math.abs(testV.dotProd(v1.subtract(v2))) < 0.0000001) return false;
		...
		return (x >= 0D && x <= p2.subtract(p1).dotProd(p2.subtract(p1)) &&
		        y >= 0.0 && y <= p3.subtract(p1).dotProd(p3.subtract(p1))); }
